"""唐怀仁 P1 榜单图（Pillow，无 Playwright）."""
...
from .compute import ThrP1Result, ThrPlayerStat
...
def _build_canvas(data: ThrP1Result, file_name: str, saohua: str, anonymous: bool) -> _Canvas:
    cv = _Canvas()
    cv.line("唐怀仁 P1 战斗统计", cv.font_title, "#222222")
    assert data.meta.p1 is not None
    cv.line(
        f"{file_name} | {data.meta.server} | P1 {data.battle_time:.1f}s | 切分：转阶段喊话",
        cv.font_sm,
        "#666666",
    )
    if data.end_shout:
        cv.line(f"「{data.end_shout}」", cv.font_sm, "#888888")
    cv.line(
        f"团队 DPS {data.team_dps:,}/s  ·  HPS {data.team_hps:,}/s",
        cv.font_sm,
        "#444444",
    )
    for title, rate_key, total_key, color in _SECTIONS:
        cv.section(title)
        cv.rank_rows(data.players, rate_key, total_key, color, anonymous)
    cv.footer(f"唐怀仁 JCL 本地分析 · jx3bla 统计 | Inkar-Suki：{saohua}")
    return cv


async def render_thr_image(data: ThrP1Result, file_name: str, saohua: str, anonymous: bool = False):
