"""唐怀仁 P1 统计：流式喂入 jx3bla CombatTracker（不保留全量 log）."""
...
from .parser import ThrJCLMeta, ThrPlayerInfo, battle_time_sec, iter_p1_combat_events, scan_thr_meta
...
class ThrP1Result:
    meta: ThrJCLMeta
    battle_time: float
    players: list[ThrPlayerStat] = field(default_factory=list)
    team_dps: int = 0
    team_hps: int = 0
    end_shout: str = ""


def _build_jx3_info(meta: ThrJCLMeta) -> OverallData:
    info = OverallData()
...
def _refine_occ(path: str, meta: ThrJCLMeta, occ: dict[str, str]) -> None:
    assert meta.p1 is not None
    for ts, et, payload in iter_p1_combat_events(path, meta.p1):
        if et == "21":
...
def compute_thr_p1(path: str) -> ThrP1Result:
    meta = scan_thr_meta(path)
    assert meta.p1 is not None
    bt = battle_time_sec(meta.p1)
    ...
    battle_ms = int(meta.p1.end_ms - meta.p1.start_ms)
...
def to_save_payload(result: ThrP1Result) -> tuple[dict[str, dict[str, int | str]], dict[str, dict[str, int | str]]]:
    dps_data: dict[str, dict[str, int | str]] = {}
    hps_data: dict[str, dict[str, int | str]] = {}
    for p in result.players:
        dps_data[p.name] = {
            "kungfu_id": p.xf_id,
            "total_damage": p.total_damage,
            "damage_per_second": p.dps,
        }
        hps_data[p.name] = {
            "kungfu_id": p.xf_id,
            "total_health": p.total_heal,
            "health_per_second": p.hps,
        }
    return dps_data, hps_data
